...
```rust
use fission_core::op::{Color as IrColor, Fill, PaintOp};
use fission_core::{NodeBuilder, Op};
...
The helper also does not automatically add semantics. If the visual surface conveys important meaning, wrap it in surrounding widgets that provide labels or interaction. If the surface needs custom input or hit-testing logic, move up to [`CustomNode`](/reference/widgets/custom-node).
...
[`CanvasLowerer`](/reference/widgets/canvas-lowerer), [`CustomNode`](/reference/widgets/custom-node), [`Composite`](/reference/widgets/composite), and [`Container`](/reference/widgets/container).
