...
Most app authors should not start here. If ordinary layout widgets solve the problem, use them. If you only need lightweight custom drawing, use [`canvas(...)`](/reference/widgets/canvas). Reach for `CustomNode` only when you truly need a new kind of node behavior.
...
Also do not use `CustomNode` when [`CanvasLowerer`](/reference/widgets/canvas-lowerer) or [`canvas(...)`](/reference/widgets/canvas) already fits the problem. Those helpers are narrower, which is usually a good thing.
...
[`Node`](/reference/widgets/node), [`canvas(...)`](/reference/widgets/canvas), and [`CanvasLowerer`](/reference/widgets/canvas-lowerer).
