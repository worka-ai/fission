...
on_drag_enter: Some(ctx.bind(SetHoveringUpload(true), on_set_hovering_upload)),
on_drag_leave: Some(ctx.bind(SetHoveringUpload(false), on_set_hovering_upload)),
on_drop: Some(ctx.bind(
    FilesDropped,
    (|state: &mut ComposerState, _action: FilesDropped, ctx| {
        if let Some(paths) = ctx.input.as_drop_paths() {
            state.attachments.extend(paths.iter().cloned());
        }
    }) as Handler<ComposerState, FilesDropped>,
)),
}
.build(ctx, view);
```
