...
on_change: Some(ctx.bind(
    SetProjectNameDraft(String::new()),
    on_set_project_name_draft as Handler<AppState, SetProjectNameDraft>,
)),
on_edit: Some(ctx.bind(BeginProjectRename, on_begin_project_rename)),
...
| --- | --- | --- | --- |
| `id` | `Option<WidgetNodeId>` | Stable identity for the editing field. | Defaults to `None`. |
