...
Use `ctx.bind(action, handler)` inside `build()` when the action wiring is local to the widget tree. This is the most common pattern for buttons, text fields, and local interactions.
