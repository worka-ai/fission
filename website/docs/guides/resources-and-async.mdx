...
fn on_save_requested(
    ...
    ctx.effects
        ...
        .on_ok(ctx.effects.bind(
            SaveSucceeded,
            on_save_succeeded as Handler<EditorState, SaveSucceeded>,
        ))
        .on_err(ctx.effects.bind(
            SaveFailed,
            on_save_failed as Handler<EditorState, SaveFailed>,
        ));
}
```
...
fn on_pick_file(
    ...
    ctx.effects
        ...
        .on_ok(ctx.effects.bind(
            FilePicked,
            on_file_picked as Handler<EditorState, FilePicked>,
        ))
        .on_err(ctx.effects.bind(
            FilePickFailed,
            on_file_pick_failed as Handler<EditorState, FilePickFailed>,
        ));
}
```
...
ctx.effects
    ...
    .on_started(ctx.effects.bind(
        ...
        on_language_started as Handler<EditorState, LanguageServiceStarted>,
    ))
    .on_event(ctx.effects.bind(
        LanguageServiceEvent,
        on_language_event as Handler<EditorState, LanguageServiceEvent>,
    ))
    .on_start_failed(ctx.effects.bind(
        LanguageServiceStartFailed,
        on_language_start_failed as Handler<EditorState, LanguageServiceStartFailed>,
    ));
```
...
ctx.effects
    ...
    .on_ok(ctx.effects.bind(
        CompletionsLoaded,
        on_completions_loaded as Handler<EditorState, CompletionsLoaded>,
    ))
    .on_err(ctx.effects.bind(
        CompletionRequestFailed,
        on_completion_request_failed as Handler<EditorState, CompletionRequestFailed>,
    ));
```
...
let poll_terminal = ctx.bind(
    PollTerminal,
    poll_terminal as Handler<TerminalExampleState, PollTerminal>,
);
