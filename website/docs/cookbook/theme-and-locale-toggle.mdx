...
impl Widget<SettingsState> for SettingsApp {
    fn build(&self, ctx: &mut BuildCtx<SettingsState>, view: &View<SettingsState>) -> Node {
        let light = ctx.bind(
            SetLightTheme,
            on_set_light_theme as Handler<SettingsState, SetLightTheme>,
        );
        let dark = ctx.bind(
            SetDarkTheme,
            on_set_dark_theme as Handler<SettingsState, SetDarkTheme>,
        );
        let english = ctx.bind(
            SetEnglish,
            on_set_english as Handler<SettingsState, SetEnglish>,
        );
        let spanish = ctx.bind(
            SetSpanish,
            on_set_spanish as Handler<SettingsState, SetSpanish>,
        );
