...
impl Widget<DashboardState> for DashboardScreen {
    fn build(&self, ctx: &mut BuildCtx<DashboardState>, view: &View<DashboardState>) -> Node {
        let tick = ctx.bind(
            ClockTick,
            on_clock_tick as Handler<DashboardState, ClockTick>,
        );
...
impl Widget<DashboardState> for DashboardScreen {
    fn build(&self, ctx: &mut BuildCtx<DashboardState>, view: &View<DashboardState>) -> Node {
        let on_started = ctx.bind(
            SyncStarted,
            on_sync_started as Handler<DashboardState, SyncStarted>,
        );
        let on_event = ctx.bind(
            SyncEventArrived,
            on_sync_event as Handler<DashboardState, SyncEventArrived>,
        );
        let on_start_failed = ctx.bind(
            SyncStartFailed,
            on_sync_start_failed as Handler<DashboardState, SyncStartFailed>,
        );
