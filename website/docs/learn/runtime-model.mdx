...
- `#[derive(...)]` asks Rust to generate common helper behavior automatically.
...
- explicit casts such as `as Handler<...>` are often just type hints for the compiler. At this stage, you can usually read them as "use this function as the reducer for that action."
...
The `#[derive(fission_macros::Action, ...)]` line is the framework-facing scaffolding that makes each type usable as a Fission action. The long derive list is real Rust, but for now you can mostly read it as "make this type work as a serializable action value."
...
impl Widget<NoteState> for NoteEditor {
    fn build(&self, ctx: &mut BuildCtx<NoteState>, view: &View<NoteState>) -> Node {
        let save = ctx.bind(
            SavePressed,
            on_save_pressed as Handler<NoteState, SavePressed>,
        );
            ...
            children: vec![
                ...
                TextInput {
                    ...
                    on_change: Some(ctx.bind(
                        ...
                        on_update_draft as Handler<NoteState, UpdateDraft>,
                    )),
                    ..Default::default()
                }
                .into_node(),
...
- `as Handler<NoteState, UpdateDraft>` and `as Handler<NoteState, SavePressed>` are explicit type hints. They help Rust understand which reducer function should handle each action. At this stage, you can read those casts as framework scaffolding rather than as business logic.
...
The most common use is `ctx.bind(...)`. Binding connects a widget event, such as a button press or text change, to an action and reducer handler. In the note editor example, `ctx.bind(UpdateDraft(...), ...)` creates the action path that lets typing update state.

This is also the place where Rust type hints can look noisier than the underlying idea. A call such as `ctx.bind(UpdateDraft(String::new()), on_update_draft as Handler<...>)` is really doing one simple thing: it tells the runtime, "when this event happens, dispatch an `UpdateDraft` action and send it to `on_update_draft`."
...
fn on_save_pressed(
    ...
    ctx.effects
        ...
        .on_ok(ctx.effects.bind(
            SaveSucceeded,
            on_save_succeeded as Handler<NoteState, SaveSucceeded>,
        ))
        .on_err(ctx.effects.bind(
            SaveFailed,
            on_save_failed as Handler<NoteState, SaveFailed>,
        ));
}
```
