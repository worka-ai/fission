...
use fission_ir::{ActionEntry, ActionSet, LayoutOp, NodeId, Op, Role, Semantics, StructuralOp};
use fission_widgets::{Button, Desugar, LoweringContext, Node, Row, Text};
...
#[test]
fn test_button_widget_desugar_without_semantics_still_wires_action() {
    let button_widget = Button {
        id: None,
        child: None,
        on_press: Some(TestClickAction { value: 99 }.into()),
        semantics: None,
        ..Default::default()
    };

    let mut cx = LoweringContext::new();
    let node_id = button_widget.desugar(&mut cx);

    let semantics_node = cx
        .ir
        .nodes
        .get(&node_id)
        .expect("semantics node should exist when on_press is set");

    if let Op::Semantics(s_op) = &semantics_node.op {
        assert_eq!(s_op.role, Role::Button);
        assert_eq!(s_op.actions.entries.len(), 1);
        assert_eq!(
            s_op.actions.entries[0].action_id,
            TEST_CLICK_ACTION_ID.as_u128()
        ...
        panic!("expected semantics node when on_press is set");
    }
}
