...
use fission_core::{Action, ActionId, AppState, ActionEnvelope, NodeId};
...
fn update_text_reducer(state: &mut MyAppState, action: &ActionEnvelope, _target: NodeId) -> Result<()> {
...
fn test_end_to_end_flow() {
    let mut harness = TestHarness::new(MyAppState::default())
        .with_root_widget(Text { content: TextContent::Literal("Initial".into()), ..Default::default() })
    ...
    // Dispatch action
    harness.dispatch(UpdateTextAction { new_text: "Updated".into() }).expect("Dispatch failed");
