...
Desktop, web, and mobile are runtime shells: they keep the app running in a host event loop. The terminal shell is also a runtime shell, but its renderer targets terminal cells instead of a GPU-backed window. The static site shell is different: it runs at conversion time, lowers real Fission widgets to Core IR, and emits static HTML, CSS, and site assets.
