...
Examples include opening a web address, opening a user-granted file picker, capturing a photo, scanning a barcode, requesting geolocation, or asking the host for biometric authentication. Fission ships low-level built-ins such as `OPEN_URL` and `PICK_OPEN_FILES` plus platform capability groups for notifications, deep links, NFC, biometrics, barcode scanning, camera and flashlight, clipboard, geolocation, haptics, microphone, Bluetooth, Wi-Fi, and volume control.
...
For reducer-side jobs, services, and commands, continue to [Commands, services, and jobs](/reference/core/commands-services-jobs). For built-in device and host capability groups, see [Platform capabilities](/reference/core/platform-capabilities/). For the build-side contract that declares resources, pair this page with [Widget trait](/reference/core/widget-trait). For the broader async teaching guide, see [Resources and async](/docs/guides/resources-and-async).
