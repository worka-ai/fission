...
Each `[[site.nav]]` entry adds one top-level navigation link to the generated documentation template. A link can also contain nested `children`, which the static-site shell renders as dropdown menus. Nested children can continue recursively, so a top-level product menu can contain a Blog item, and Blog can contain individual post links.
