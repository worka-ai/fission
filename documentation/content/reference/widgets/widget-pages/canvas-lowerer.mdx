...
| `painter` | `Arc<dyn Fn(&mut LoweringContext) -> Vec<WidgetId> + Send + Sync>` | Closure that emits child node ids directly into the lowering context. | Required. This is the advanced part of the public contract. |
...
That means it is best for lightweight custom drawing helpers. If the surface also needs pointer handling, text input, or custom hit testing, you are usually in [`CustomWidget`](/reference/widgets/custom-widget) territory rather than plain `CanvasLowerer` territory.
