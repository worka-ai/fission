...
| --- | --- | --- | --- |
| `id` | `Option<WidgetId>` | Stable node identity. | Defaults to `None`, so the runtime generates an id. |
| `child` | `Widget` | The node to center. | Required. `Align` is a single-child layout wrapper. |
