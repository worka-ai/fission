...
Service latency trend belongs to the **Line** family. Its job is to make this data shape readable: `Latency samples over ordered time buckets.` In a production interface, choose this chart when the visual form makes the user's question faster to answer than a table or a simpler chart would.
