...
Resource saturation belongs to the **Heatmap** family. Its job is to make this data shape readable: `Resource and time values on a matrix.` In a production interface, choose this chart when the visual form makes the user's question faster to answer than a table or a simpler chart would.
