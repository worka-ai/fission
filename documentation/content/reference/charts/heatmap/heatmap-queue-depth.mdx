...
Queue depth matrix belongs to the **Heatmap** family. Its job is to make this data shape readable: `Queue depth by queue and time bucket.` In a production interface, choose this chart when the visual form makes the user's question faster to answer than a table or a simpler chart would.
