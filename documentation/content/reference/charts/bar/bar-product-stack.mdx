---
title: Product stack bars
sidebar_label: Product stack bars
description: Fission Charts reference for Product stack bars.
---

# Product stack bars

![Product stack bars screenshot](/img/charts/bar-product-stack.png)

Product stack bars uses Fission Charts typed Rust data to render a production-ready bar view. Use it for portfolio composition. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Product stack bars belongs to the **Bar** family. Its job is to make this data shape readable: `Product values stacked into category totals.` In a production interface, choose this chart when the visual form makes the user's question faster to answer than a table or a simpler chart would.

Avoid it when the visual form makes the user estimate more than necessary; choose the simplest chart that answers the product question.

## Data model

`Product values stacked into category totals.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.

## Rust API

| Field | Type | Notes |
| --- | --- | --- |
| `title` | `&str` | Names the chart for the screen, accessibility tree, and test output. |
| `x_axis` | `Axis` | Usually `Axis::category`; use `Axis::value()` when x positions are numeric. |
| `y_axis` | `Axis::value()` | Maps each sample value to the chart range. |
| `series` | `Series` | Provides the typed data model for this chart family. |
| `width / height` | `f32` | Optional fixed size; omit them when the chart should flex inside Fission layout. |

## Example

```rust
use fission::charts::{Axis, BarSeries, Chart};

let chart = Chart::new()
    .title("Orders")
    .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
    .y_axis(Axis::value())
    .series(vec![BarSeries::new("Orders")
        .data(vec![120.0, 156.0, 184.0, 212.0])
        .into()]);
```

## Interaction and animation

Charts can emit typed `ChartInteractionEvent` values when interaction is enabled. Handle those events in a reducer when the app needs hover, press, release, scroll, selection, or brush behavior. `ChartAnimation` stores duration, delay, stagger, easing, and reduced-motion behavior as deterministic chart data, so animation timing can be tested instead of being hidden in ad-hoc timers.

## Testing guidance

For this chart, test the data mapping first, then test lowering, then capture a live screenshot when visual output changes. The screenshot for this page is refreshed by running the ignored chart-gallery screenshot capture test with `FISSION_UPDATE_CHART_DOCS=1`, which updates `documentation/static/img/charts` before the static site build.

Tags: `bar`, `product`.


[Bar family overview](./overview)
